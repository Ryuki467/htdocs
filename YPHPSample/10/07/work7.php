...
if(!empty($_SESSION)){
	...
	if(isset($_SESSION["naisho"])){
		$naisho = $_SESSION["naisho"];
	}
}
...
if(!empty($_POST)){
	...
	if(isset($_POST["naisho"])){
		$naisho = $_POST["naisho"];
		$_SESSION["naisho"] = $naisho;
	}
}
...
<form action = "work7.php" method ="post">
	...
	<table>
		...
		<tr>
			<th>コメント</th>
			<td><textarea class = "form-control" id = "comment" name  = "comment" rows = "4" cols = "40"><?php echo "{$comment}"; ?></textarea></td>
		</tr>
		<tr>
			<th>内緒</th>
			<td>
				<input type = "hidden" name  = "naisho" value = "これは画面に表示されません" />
