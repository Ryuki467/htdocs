...
<form action="http://localhost/YPHPSample/Rensyu/11/Sample8.php" method="post">
...
while($value = $data->fetch()){
	...
	$name = $value["name"];
	$price = $value["price"];
	echo"<tr><td><input type=\"radio\" name=\"delid\" value=\"{$id}\" /></td><td>{$id}</td><td>{$name}</td><td>{$price}</td></tr>\n";
}
$db = null;
?>
</table>
