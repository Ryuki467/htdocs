...
abstract class Application
{
    ...
    protected function initialize()
    {
        ...
        //$this->registerRoutes()　ZaisekiApplication　のregisterRoutes関数に記載のある内容全て
    ...
    public function run()
    {
        ...
        try {
            //falseもしくはZaisekiApplicationの値が代入される
            //:actionの部分にMessageControllerの$auth_actionsが代入されているのでは？
            $params = $this->router->resolve($this->request->getPathInfo());
            if ($params === false) {
                //例外を送信
                throw new HttpNotFoundException('No route found for ' . $this->request->getPathInfo());
            }
            //ZaisekiApplicationのcontroller
            $controller = $params['controller'];
            
            //ZaisekiApplicationのaction
    ...
    public function runAction($controller_name, $action, $params = array())
    {
        ...
        //$controller_name　contllor名前 message,status,userなど
        $controller_class = ucfirst($controller_name) . 'Controller';
        //controller_classのファイルが存在すればrequire_onceして、$controller_class($this)を返す
        $controller = $this->findController($controller_class);

        if ($controller === false) {
            throw new HttpNotFoundException($controller_class . ' controller is not found.');
        }
        //$action ZaisekiApplicationのactionの値(index,addなど)が入る?
        //ここでclassを実行
    ...
    {
        //class_exists　クラスが定義済みかどうか確認する
        if (!class_exists($controller_class)) {
            $controller_file = $this->getControllerDir() . '/' . $controller_class . '.php';
            // $controller_file C:\xampp\htdocs\PH2\zaiseki/controllers/StatusController.phpなど
